...
If your hot tub does not have a ACE, Freshwater or Freshwater IQ modules, you can have the ESP32 emulate a ACE module on your spa remote. It will allow you to control the salt level from 0 to 10, turn on/off the boost mode, press the test button and see a status bar from 0 to 7. When enabled, all these values will be sent to Home Assistant to be used for automations much like the [audio module emulation](https://github.com/Ylianst/ESP-IQ2020/blob/main/documentation/audio.md). Note that we you have any of these optional modules (ACE, Freshwater, Freshwater IQ) enabling this will interfere and/or case issues.

To enable this feature, add the `ace_emulation: true` to the `IQ2020` section like this:
